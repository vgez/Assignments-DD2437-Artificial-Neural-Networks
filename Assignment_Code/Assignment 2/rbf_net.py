...
class RBF():
    ...
    def __init__(self, input_size, rbf_size, output_size, lower_range, upper_range):
    ...
    def calc_err(self, x_data, y_data):
        ...
        return np.sum(np.mean(np.abs(y_data - pred)))

    def update(self, err, phi):
        self.weights = np.add(self.weights, np.reshape(
            self.eta*err*np.transpose(phi), np.shape(self.weights)))
    ...
    def calc_phi(self, data, test=False):
        ...
        if not self.batch:
            ...
            for i in range(self.rbf_size):
                phi[i] = self.gauss_rbf(self.mu[i], self.var, data[i])
        return phi
        ...
        if self.batch:
            ...
            for epoch in range(1):
                for i in range(len(self.x_train)):
                    f_hat = np.dot(phi[i, :], self.weights)
                    err = self.calc_err_seq(f_hat, i)
                    self.update(err, phi[i, :])
