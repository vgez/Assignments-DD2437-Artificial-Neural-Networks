...
class RBF():
    ...
    def calc_phi(self, data, test=False):
        ...
        if self.batch:
            ...
            print(self.mu.shape)
            phi = np.zeros(
                (self.x_train.shape[0], self.rbf_size))
            for i in range(self.x_train.shape[0]):
                for j in range(self.rbf_size):
                    testing = self.gauss_rbf(
                        self.mu[j, :], self.var, data[i, :])
                    print(testing)
                    phi[i, j] = testing
        else:
            phi = np.zeros((self.rbf_size, self.input_size))
    ...
    def train(self, epochs=2, batch=False, eta=0.002, competitive=True, noise=False, ballist=True):
        """
...
def main():
    RBF_net = RBF(2, 10, 2, 0, (2*np.pi), 0.3)
    RBF_net.train()
